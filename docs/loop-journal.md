...
Iteration 3b: doing the R0 parser/persistence hardening from the 2026-07-20 external audit, touching `backend/app/scrapers/biznesradar.py`, `backend/app/services/refresh.py`, `backend/app/services/publication_dates.py`, `backend/tests/test_biznesradar_parser.py`, `skills/scraper-doctor/SKILL.md`, `docs/loop-journal.md`, `CHANGELOG.md`, and `docs/model-usage.md`, done when a publication row whose value-cell count differs from the header's yields all-`None` dates (both drift shapes tested), the dead `IGNORED_ROW_LABELS` path is deleted with the parser-side exclusion still asserted, all 568 real-fixture period/date pairs are value-checked in CI, the backfill docstring names its bounded failure classes, and the full backend suite passes.

Iteration 3b drift check:

- a. Authorized by R0 ("fixture + tests per scraper-doctor") and by the owner's explicit 2026-07-20 direction to implement the external audit's M1/M2/L1/L3 findings directly.
- b. It preserves APPROACH's process-at-T requirement and V5/V9/V10: misaligned availability metadata degrades to explicit `not_reported` facts rather than wrong dates; the strongest fixture verification is now computed in CI rather than asserted out-of-band; the duplicate metadata-exclusion path is deleted, leaving one canonical parser path.
- c. The increment was not exceeded. The diff contains only the alignment guard, the dead-path deletion, the fixture value guard, one docstring, the quirks-ledger correction and ledgers; no PIT selector, coverage report, schema or UI change.
- d. Grep clean: added lines contain no author/persona branding, probabilities, target prices or `latest` reads; `IGNORED_ROW_LABELS` has zero remaining references.
- e. No test was weakened, skipped or deleted; 13 test cases were added (1 drift-shape test, 12 fixture-value cases). Full backend suite: 414 passed (401 before this slice). `git diff --check`: clean.

Iteration 3b verification: owner-authorized exception to drafter/verifier separation — the external audit session (Claude Fable 5, claude.ai) specified, implemented and verified this slice in one context; compensating evidence is computed, not attested. The audit's adversarial probes B/C (extra leading cell, colspan label) return all-`None` on rerun; the 568 fixture pairs the guard asserts were recomputed independently from the committed captures before the guard existed and match one-to-one, including the live-backfill cross-check (SNT fixtures = 215 pairs = 215 persisted facts); the full 414-test suite passed in a clean container. The next independent verifier context should re-run `tests/test_biznesradar_parser.py` and confirm zero `IGNORED_ROW_LABELS` references before building on this slice.

Heartbeat: Iteration 3b complete; the next unblocked R0 increment remains the PIT-coverage report per company/period.
