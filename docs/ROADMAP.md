...
Portfolio; Portfolio correctly excludes the current provisional valuation from
aggregation.
...
| 3 | S4 Portfolio precision + auto-coverage | Real TWR/XIRR, operation-history import, mapping and reconciliation gates pass; sync queues canonical coverage by weight × staleness; only current verified v3 valuations aggregate | Deterministic math: no model. Implementation and ordinary portfolio interpretation: `gpt-5.6-terra` medium. Complex cross-company synthesis: `gpt-5.6-sol` high. Strict verification: `gpt-5.6-sol` high | in progress · live sync, TWR/XIRR, mapping, 99.97% value coverage, priority queueing, current-only nullable scenario aggregation and in-app browser acceptance pass; preview-confirm CSV operation import is implemented and zero-write previewed, but Kuba has not supplied a real full export |
| 4 | S6 Outcome scoring | First actual report scores direction, range hit and calibration against a canonical v3 valuation; result visible per engine version | Scoring: no model. Explanation/UI: `gpt-5.6-terra` medium. Method review: `gpt-5.6-sol` high | blocked · the only canonical valuation is dated 2026-07-15 and every stored financial actual was already known before it; no post-valuation report exists to score |
| 5 | S7 Report-calendar awareness | Report dates drive idempotent re-Research/re-Valuation around publication | Collection/normalization: no model. Calendar wiring: `gpt-5.6-terra` medium. Mechanical tests: `gpt-5.3-codex-spark` when available, else `gpt-5.6-luna` low with the fallback recorded | in progress · independent verification accepts source parsing/lineage, post-refresh frozen scheduling, due-time claims, manual acceleration, visible gaps and Research→Valuation handoff; live SNT shows 2026-08-05 but its Codex-proposed profile requires Kuba's confirmation before the future job may exist |
...
| S4 | Portfolio precision + auto-coverage | Current execution order 3 | in progress · `portfolio-performance-v1` independently computes live TWR/XIRR; one canonical resolver preserves precise GPW identities; successful sync #4 retained nine rows and queued/reused canonical coverage by weight × staleness; latest-only scenario aggregation preserves deterministic sensitivity while withholding uncalibrated weighted value; `myfund-operations-csv-v1` provides zero-write preview, sequence-bound confirmation, timestamp/tax-aware parsing, flow reconciliation and quantity/order-gated cost-basis reconstruction. Cross-stage in-app browser acceptance now passes; only a real full operations export remains required |
...
| S6 | Outcome scoring (learning loop v1) | first scenario-outcome job scores a valued company against an actual report; calibration visible per engine version | blocked · no stored financial actual postdates the only canonical valuation (2026-07-15), so direction/range/calibration cannot yet be computed honestly |
| S7 | Report-calendar awareness | holdings' next report dates tracked; re-research/re-valuation queued around publication | in progress · independent verification accepts `report-calendar-v1`: it validates immutable profile lineage, freezes the review after the full bounded refresh, displays dates/gaps, keeps future jobs unclaimable, reuses manual review and hands usable Research to canonical Valuation. Live SNT is source-proven but human-profile-blocked, so the real eligible-path gate remains open |
...
- Kuba must provide a full, unfiltered myfund operation-history CSV to complete
  the live V7 reconciliation gate; the configured read API exposes current
  portfolio state/daily series, not operation history.
- BiznesRadar premium session for market pages where anonymous truncates.
- Kuba must confirm or correct SNT's current profile before its source-backed
  2026-08-05 report schedule can create cost-bearing Research/Valuation work.
- S6 requires the first actual company report published after a canonical
  valuation; current stored financial facts all predate the 2026-07-15 valuation.
