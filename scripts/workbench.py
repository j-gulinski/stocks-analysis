...
def status_payload() -> dict[str, Any]:
    ...
    return {
        "postgres": {"port": 5432, "listening": _port_open(5432)},
        "backend": backend,
        "frontend": frontend,
...
def doctor_checks() -> list[Check]:
    ...
    checks.append(
        Check(
            ...
            "listening on 127.0.0.1:5432" if services["postgres"]["listening"] else "not running; ./workbench start will start docker compose postgres",
        )
    )
...
def _start_postgres() -> Check:
    ...
    while time.monotonic() < deadline and not _port_open(5432):
        time.sleep(0.5)
    if not _port_open(5432):
        return Check("postgres", "fail", "container started but port 5432 did not become ready")
    ...
    if migration is None or migration.returncode != 0:
        ...
        return Check("postgres", "fail", detail)
    return Check("postgres", "pass", "Docker Postgres ready; migrations at head")
...
def _print_status(payload: dict[str, Any], as_json: bool) -> None:
    ...
    print(f"PostgreSQL: {postgres} on 127.0.0.1:5432")
